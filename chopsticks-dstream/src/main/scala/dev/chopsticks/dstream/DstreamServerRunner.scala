...
import zio.{RIO, Task, UIO, URIO, URLayer, ZIO, ZManaged}
  ...
  def live[In: zio.Tag, Assignment: zio.Tag, Result: zio.Tag, Out: zio.Tag]: URLayer[
    ...
    val manageable = ZManageable { (config: DstreamServerConfig) =>
      for {
        ...
        flowFactoryManaged = ZRunnable
          .apply { args: (In => UIO[Assignment], (In, WorkResult[Result]) => Task[Out]) =>
            val (createAssignment, handleResult) = args
        ...
        flowFactory <- flowFactoryManaged
