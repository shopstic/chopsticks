...
abstract class ColumnFamily[K: KeySerdes, V: ValueSerdes] {
  lazy val id: String = SnakeCase.fromTokens(PascalCase.toTokens(this.getClass.getSimpleName))
  ...
  def deserializeKey(bytes: Array[Byte]): DbKeyDecodeResult[K] = keySerdes.decode(bytes)
