...
// todo unify it with FDB ColumnFamily
abstract class ColumnFamily[K: KeySerdes, V: ValueSerdes](val id: String):
  type Key = K
  type Value = V

  def keySerdes: KeySerdes[K] =
    summon[KeySerdes[K]]
  def valueSerdes: ValueSerdes[V] =
    summon[ValueSerdes[V]]
