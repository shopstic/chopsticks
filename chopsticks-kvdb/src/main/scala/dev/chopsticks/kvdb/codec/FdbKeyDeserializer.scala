...
import com.apple.foundationdb.tuple.{ByteArrayUtil, Versionstamp}
...
trait FdbKeyDeserializer[T] extends KeyDeserializer[T] {
  def deserialize(in: FdbTupleReader): Either[DecodingFailure, T]
  final def deserialize(bytes: Array[Byte]): KeyDeserializer.KeyDeserializationResult[T] =
    val reader =
      try {
        Right(new FdbTupleReader(com.apple.foundationdb.tuple.Tuple.fromBytes(bytes)))
      }
      catch {
        case NonFatal(e) =>
          Left(DecodingFailure.GenericKeyDeserializationException(
            s"""
    ...
    reader.flatMap(deserialize)
}

object FdbKeyDeserializer extends Derivation[FdbKeyDeserializer] {
  type Typeclass[A] = FdbKeyDeserializer[A]

  private[codec] def createTry[T](f: FdbTupleReader => T)(using ct: ClassTag[T]): FdbKeyDeserializer[T] =
    (in: FdbTupleReader) => {
      try Right(f(in))
      catch {
        case NonFatal(e) =>
          Left(
            DecodingFailure.GenericKeyDeserializationException(
              s"""
                 |Failed decoding to ${ct.toString()}: ${e.toString}
                 |------------------------------------------------
                 |Details: ${in.toString}
                 |------------------------------------------------
                 |""".stripMargin,
  ...
  implicit val byteStringFdbKeyDecoder: FdbKeyDeserializer[ByteString] =
    createTry(t => ByteString.copyFrom(t.getBytes))
...
//    typ: Typeable[T]
    ...
    (in: FdbTupleReader) =>
      underlyingDecoder.deserialize(in).flatMap { u =>
        try Right(comp.fromValue(u))
          ...
          case NonFatal(e) =>
            Left(DecodingFailure.GenericKeyDeserializationException(
              s"Failed decoding to proto enum ${ct.toString}: ${e.toString}",
              e
            ))
        }
      }
  ...
  implicit def enumeratumByteEnumKeyDecoder[E <: ByteEnumEntry](implicit
    ...
    Try(e.withValue(in.getBigInteger.byteValueExact())) match
      case Failure(e) => Left(DecodingFailure.GenericKeyDeserializationException(e.getMessage, e))
  ...
  implicit def refinedFdbKeyDeserializer[F[_, _], T, P](implicit
    ...
    deserializer.deserialize(in).flatMap { value =>
      refType.refine[P](value).leftMap(DecodingFailure.GenericKeyDeserializationException(_))
    }
...
//  implicit def gen[T]: FdbKeyDeserializer[T] = macro Magnolia.gen[T]
  ...
  def join[A](ctx: CaseClass[FdbKeyDeserializer, A]): FdbKeyDeserializer[A] = (in: FdbTupleReader) =>
    ctx.constructMonadic { param => param.typeclass.deserialize(in) }

  def split[A](ctx: SealedTrait[FdbKeyDeserializer, A]): FdbKeyDeserializer[A] = (in: FdbTupleReader) =>
    ???
