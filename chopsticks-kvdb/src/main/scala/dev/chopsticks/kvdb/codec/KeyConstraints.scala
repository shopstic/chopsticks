...
object KeyConstraints:
  ...
  def constraint[K: KeySerdes](c: KeyConstraints[K] => KeyConstraints[K]): KvdbKeyConstraintList =
    toList(c(seed))
  ...
  object Implicits:
    implicit val dbKeyConstraintShow: Show[KvdbKeyConstraint] =
      Show.show { r =>
    ...
    implicit val dbKeyConstraintListShow: Show[List[KvdbKeyConstraint]] =
      Show.show { r => r.map(_.show).mkString(" AND ") }

    implicit val dbKeyRangeShow: Show[KvdbKeyRange] =
      Show.show { r => s"KvdbKeyRange(from ${r.from.show} -> to ${r.to.show})" }
  end Implicits

  extension (builder: KeyConstraints[String])
    def literallyStartsWith(value: String)(using e: KeyPrefix[String, String]): KeyConstraints[String] =
...
final case class KeyConstraints[K](constraints: Queue[KvdbKeyConstraint] = Queue.empty)(implicit
  ...
  def gtEq[P](v: P)(using e: KeyPrefix[P, K]): KeyConstraints[K] =
    copy(
  ...
  def >=[P](v: P)(using e: KeyPrefix[P, K]): KeyConstraints[K] =
    gtEq(v)

  def gt[P](v: P)(using e: KeyPrefix[P, K]): KeyConstraints[K] =
    copy(
      constraints enqueue KvdbKeyConstraint(
        Operator.GREATER,
        ProtoByteString.copyFrom(serdes.serializePrefix(v)),
  ...
  def ltEq[P](v: P)(using e: KeyPrefix[P, K]): KeyConstraints[K] =
    copy(
      constraints enqueue KvdbKeyConstraint(
        Operator.LESS_EQUAL,
        ProtoByteString.copyFrom(serdes.serializePrefix(v)),
  ...
  def lt[P](v: P)(using e: KeyPrefix[P, K]): KeyConstraints[K] =
  ...
  def lastStartsWith[P](v: P)(using e: KeyPrefix[P, K]): KeyConstraints[K] =
  ...
  def startsWith[P](v: P)(using e: KeyPrefix[P, K]): KeyConstraints[K] =
    copy(constraints enqueue KvdbKeyConstraint(
      Operator.PREFIX,
      ProtoByteString.copyFrom(serdes.serializePrefix(v)),
      v.toString
  ...
  def startsWithLiterally[P](v: P)(using e: KeyPrefix[P, K]): KeyConstraints[K] =
    copy(constraints enqueue KvdbKeyConstraint(
      Operator.GREATER_EQUAL,
      ProtoByteString.copyFrom(serdes.serializeLiteralPrefix(v)),
      v.toString
    ))

  def lastStartsWithLiterally[P](v: P)(using e: KeyPrefix[P, K]): KeyConstraints[K] =
    copy(constraints enqueue KvdbKeyConstraint(
      operator = Operator.LESS_EQUAL,
      operand = ProtoByteString.copyFrom(KvdbUtils.strinc(serdes.serializeLiteralPrefix(v))),
      operandDisplay = v.toString
    ))
