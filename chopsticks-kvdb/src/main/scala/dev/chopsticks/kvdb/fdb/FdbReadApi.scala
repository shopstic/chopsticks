...
class FdbReadApi[CFS <: ColumnFamily[_, _]](
  val tx: ReadTransaction,
  dbContext: FdbContext[CFS]
) {
  def snapshot(): FdbReadApi[CFS] =
    new FdbReadApi[CFS](tx.snapshot(), dbContext)
  ...
  def addReadConflictKeyIfNotSnapshot[Col <: ColumnFamily[_, _]](column: Col, key: Array[Byte])(using
    CFS <:< Col
  ): Boolean =
    tx.addReadConflictKeyIfNotSnapshot(dbContext.prefixKey(column, key))

  def addReadConflictRangeIfNotSnapshot[Col <: ColumnFamily[_, _]](
    column: Col,
    begin: Array[Byte],
    end: Array[Byte]
  )(using CFS <:< Col): Boolean =
    tx.addReadConflictRangeIfNotSnapshot(dbContext.prefixKey(column, begin), dbContext.prefixKey(column, end))

  def getEither[Col <: ColumnFamily[_, _]](
    column: Col,
    constraints: List[KvdbKeyConstraint]
  )(using CFS <:< Col): CompletableFuture[Either[Array[Byte], KvdbPair]] =
    getEitherByColumnId(column.id, constraints)

  private def doGetRangeFuture[Col <: ColumnFamily[_, _]](
    column: Col,
    from: List[KvdbKeyConstraint],
    to: List[KvdbKeyConstraint],
    limit: PosInt,
    reverse: Boolean
  )(using CFS <:< Col): CompletableFuture[List[KvdbPair]] = {
    ...
    tx.getRange(startKeySelector, endKeySelector, limit.value, reverse)
      .asList()
      .thenApply { javaList =>
  ...
  def get[Col <: ColumnFamily[_, _]](
    column: Col,
    constraints: List[KvdbKeyConstraint]
  )(using CFS <:< Col): CompletableFuture[Option[KvdbPair]] =
  ...
  def getRange[Col <: ColumnFamily[_, _]](
    column: Col,
    range: KvdbKeyRange,
    ...
    PosInt.from(range.limit) match
      ...
      case Right(limit) =>
        doGetRangeFuture(column, range.from, range.to, limit, reverse)
