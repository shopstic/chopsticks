...
import dev.chopsticks.kvdb.util.TestZioProbeOps.*
...
object KvdbDatabaseTest extends Matchers with Inside {
  private def flattenFlow[T]: ZPipeline[Any, Nothing, Chunk[T], T] =
    StreamUtils.Pipeline.mapConcatChunk { (b: Chunk[T]) =>
      b
    }
  ...
  private val collectValuesSink: ZSink[Any, Nothing, KvdbValueBatch, Nothing, KvdbValueBatch] =
    flattenFlow[Array[Byte]].toSink(ZSink.collectAll)

  private val collectSink: ZSink[Any, Nothing, KvdbBatch, Nothing, KvdbBatch] =
    flattenFlow[KvdbPair].toSink(ZSink.collectAll)

  private def collectPairs(
    source: Stream[Throwable, KvdbBatch]
  ): Task[KvdbBatch] =
    source
      .run(collectSink)
      .uninterruptible
  ...
  private def collectValues(
    source: Stream[Throwable, KvdbValueBatch]
  ): Task[KvdbValueBatch] =
    source
      .run(collectValuesSink)
      .uninterruptible

  private[kvdb] val $ : (KeyConstraints[String] => KeyConstraints[String]) => KvdbKeyConstraintList =
    KeyConstraints.constraint[String]
...
abstract private[kvdb] class KvdbDatabaseTest
    ...
    with Inside
    with KvdbTestSuite {
  import KvdbDatabaseTest.*

  protected def managedDb: ZIO[KvdbIoThreadPool with Scope, Throwable, TestDatabase.Db]
  ...
  private lazy val withDb: (TestDatabase.Db => RIO[KvdbIoThreadPool with Scope, Assertion]) => Future[Assertion] =
    createTestRunner(managedDb) { effect =>
      ZIO.scoped {
  ...
  "iterateSource" should {
    ...
    "respect given MaxKvdbBatchBytes" in withDb { db =>
      ...
      for {
        ...
        batches <- db
          .withOptions(_.copy(batchReadMaxBatchBytes = maxBatchBytes))
  ...
  "tailSource" should {
    "complete with UnsupportedKvdbOperationException if constraint list is empty" in withDb { db =>
      db
        .tailSource(defaultCf, $$(identity, identity))
        .runHead
        .uninterruptible
        .either
        .map { ret =>
    ...
    "respect given MaxKvdbBatchBytes" in withDb { db =>
      ...
      for {
        ...
        batches <- db.withOptions(_.copy(batchReadMaxBatchBytes = maxBatchBytes))
          ...
          .run(ZSink.collectAllWhile { b =>
            b.forall(a => KvdbSerdesUtils.byteArrayToString(a.last._2) != "10000")
          })
          .uninterruptible
      } yield {
        batches.size should be < count
    ...
    "tail" in withDb { db =>
      for {
        killSwitch <- zio.Promise.make[Throwable, Unit]
        source = db
        ...
        probe <- source
          ...
          .toTestProbe
        _ <- db.putTask(defaultCf, "aaaa1", "aaaa1")
        _ <- probe.expectNoMessage(zio.Duration.fromMillis(100))
        _ <- db.putTask(defaultCf, "bbbb1", "bbbb1")
        _ <- probe.expectMsg(zio.Duration.fromSeconds(3)).flatMap { case (k, v) =>
          ZIO.attempt {
            byteArrayToString(k) should equal("bbbb1")
            byteArrayToString(v) should equal("bbbb1")
          }
        }
        _ <- db.putTask(defaultCf, "cccc1", "cccc1")
        _ <- probe.expectNoMessage(zio.Duration.fromMillis(100))
        _ <- db.putTask(defaultCf, "bbbb2", "bbbb2")
        lastPair <- probe.expectMsg(zio.Duration.fromSeconds(3))
        _ <- killSwitch.complete(ZIO.unit)
        _ <- probe.expectStreamCompletion(zio.Duration.fromMillis(300))
      } yield {
    ...
    "tail last when empty" in withDb { db =>
      for {
        ...
        pair <- probe.expectMsg(1.second)
        _ <- probe.expectStreamCompletion(300.millis)
      } yield {
    ...
    "tail last when not empty" in withDb { db =>
      for {
        ...
        head <- db
          ...
          .collect { case Right(b) => b }
          .runHead
          .timeoutFail(new TimeoutException(s"Element didn't appear within the specified duration"))(1.second)
          .map(_.get.head)
      } yield {
  ...
  "batchTailSource" should {
    "complete with UnsupportedKvdbOperationException if constraint list is empty" in withDb { db =>
      db
        .concurrentTailSource(defaultCf, List.empty)
        .runHead
        .map(_.get)
        .either
        .map { ret =>
    ...
    "tail" in withDb { db =>
      for {
        ...
        probe <- db
          ...
          .collect {
            case (index, Right(b)) => (index, b.map(p => byteArrayToString(p._1) -> byteArrayToString(p._2)))
          }
          .interruptWhen(killSwitch)
          .toTestProbe
        _ <- db.putTask(defaultCf, "cccc1", "cccc1")
        ...
        _ <- db.putTask(defaultCf, "aaaa1", "aaaa1")
        lastMessage <- probe.expectMsg(3.seconds)
        _ <- killSwitch.complete(ZIO.unit)
        _ <- probe.expectStreamCompletion(300.millis)
      } yield {
  ...
  "tailValuesSource" should {
    "tail" in withDb { db =>
      for {
        ...
        probe <- db
          .tailSource(defaultCf, $$(_ ^= "bbbb", _ ^= "bbbb"))
          .collect { case Right(v) => v.map(_._2) }
          .via(flattenFlow)
          .rechunk(1)
          .interruptWhen(killSwitch)
          .toTestProbe
        ...
        _ <- probe.expectNoMessage(100.millis)
        _ <- db.putTask(defaultCf, "bbbb1", "bbbb1")
        _ <- probe.expectMsg(3.seconds).flatMap { value =>
          ZIO.attempt {
        ...
        _ <- probe.expectNoMessage(100.millis)
        _ <- db.putTask(defaultCf, "bbbb2", "bbbb2")
        lastValue <- probe.expectMsg(3.seconds)
        _ <- killSwitch.complete(ZIO.unit)
        _ <- probe.expectStreamCompletion(300.millis)
      } yield {
