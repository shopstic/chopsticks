...
// copied from scala-xml
class XmlPrettyPrinter(width: Int, step: Int, minimizeEmpty: Boolean) {
  ...
  protected def leafTag(n: Node): String = {
    def mkLeaf(sb: StringBuilder): Unit = {
      val _ = sb.append('<')
      val _ = n.nameToString(sb)
      val _ = n.attributes.buildString(sb)
      val _ = sb.append("/>")
    }
    sbToString(mkLeaf)
  }
  ...
  protected def startTag(n: Node, pscope: NamespaceBinding): (String, Int) = {
    ...
    def mkStart(sb: StringBuilder): Unit = {
      ...
      i = sb.length + 1
      val _ = n.attributes.buildString(sb)
      val _ = n.scope.buildString(sb, pscope)
      val _ = sb.append('>')
    }
  ...
  /** Appends a formatted string containing well-formed XML with given namespace to prefix mapping to the given string
    * buffer.
    *
    * @param n
  ...
  def format(n: Node, pscope: NamespaceBinding, sb: StringBuilder): Unit = { // entry point
    ...
    for (b <- items.reverse) b match {
      ...
      case Para(s) =>
        ...
        sb.append(s)
      case other =>
        throw new IllegalArgumentException("unknown item: " + other)
    }
  }
  ...
  /** Appends a formatted string containing well-formed XML with the given namespace to prefix mapping to the given
    * stringbuffer.
    *
    * @param nodes
