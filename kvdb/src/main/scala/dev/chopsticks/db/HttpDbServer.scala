...
import dev.chopsticks.db.DbWebsocketServerRequestHandlerFlow.ClientTerminatedPrematurelyException
...
//      Counter.build().name(dbServerPrefix + "wrapped_compressed_bytes").help("Wrapped Compressed Bytes").register()
  ...
  private[db] def createHandler(handler: Message => Source[Array[Byte], NotUsed]): Flow[Message, Message, NotUsed] = {
    Flow[Message]
