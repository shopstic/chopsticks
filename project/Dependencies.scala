...
object Dependencies {
  val SCALA_VERSION = "3.3.0-RC3"

  lazy val fpDeps = {
    val main = pureconfig ++ squants ++ zio ++ zioLogging ++ zioPrelude
    val test = scalatest.map(_ % Test)
    main ++ test
  }

  lazy val kvdbDeps = {
    val main = enumeratum ++ fdb ++ magnolia ++ pureconfig ++ refined ++ scalaPb ++ zio
    val test = scalatest.map(_ % Test)
    main ++ test
  }

  lazy val metricDeps = {
    val main = prometheusClient
    val test = scalatest.map(_ % Test)
    main ++ test
  }

  lazy val schemaDeps = {
    val main = pureconfig ++ refined ++ squants ++ tapir ++ typesafeConfig ++ zioSchema
  ...
  lazy val utilDeps = {
    val main = enumeratum ++ microlibsDeps ++ pureconfig ++ squants ++ zio
    val test = Seq.empty[ModuleID].map(_ % Test)
    main ++ test
  }

  lazy val testkitDeps = {
    scalatest ++ zioTest
  }

  private val enumeratum = Seq("com.beachape" %% "enumeratum-circe" % "1.7.2")
  ...
  private val magnolia = Seq("com.softwaremill.magnolia1_3" %% "magnolia" % "1.2.5")

  private val microlibsDeps = Seq(
    "com.github.japgolly.microlibs" %% "utils" % "4.2.1"
  )

  private val refined = Seq("eu.timepit" %% "refined" % "0.10.1")

  private val prometheusClientVersion = "0.16.0"
  private val prometheusClient = Seq("simpleclient", "simpleclient_common")
    .map(m => "io.prometheus" % m % prometheusClientVersion)

  private val pureconfig = Seq(
    "com.github.pureconfig" %% "pureconfig-core" % "0.17.2"
  )

  private val squants = Seq("org.typelevel" %% "squants" % "1.8.3")

  private val scalaPb = Seq(
  ...
  private val tapir = Seq("tapir-core").map(n => "com.softwaremill.sttp.tapir" %% n % "1.2.5")

  private val typesafeConfig = Seq("com.typesafe" % "config" % "1.4.2")

  private val zioSchemaVersion = "0.4.2"
  private val zioSchema = Seq("zio-schema", "zio-schema-derivation").map(n => "dev.zio" %% n % zioSchemaVersion)
  ...
  private val zioTest = Seq("zio-test", "zio-test-sbt", "zio-test-magnolia", "zio-test-junit")
    .map(x => "dev.zio" %% x % zioVersion)
  ...
  private val zioPrelude = Seq("dev.zio" %% "zio-prelude" % "1.0.0-RC16")
